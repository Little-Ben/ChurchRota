...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		<div class="elementBackground highlight">
            ...
            <? if ($row['firstName'] != "") { 
				...
				if($row['topicName'] == "") {
					...
					$resultTopicName = mysql_query($sqlTopicName) or die(mysql_error());
	
					while($rowTopicName = mysql_fetch_array($resultTopicName, MYSQL_ASSOC)) {
	...
	while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		FROM cr_discussion WHERE CategoryParent = 9 AND topicParent = '$postparent'";
			 $latestpostresult = mysql_query($latestpostsql) or die(mysql_error());
